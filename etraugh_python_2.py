...
def import_data(filename):
    ...
    #loops through every line in the csv and splits them by commas.
    #Creates a list where each element is a list made of the name, type, hp, and base damage
    participants = [line.strip('\n').split(',') for line in f.readlines()]
    #turn all hp and base damage into float
    for participant in participants:
        participant[2] = float(participant[2])
        participant[3] = float(participant[3])
    f.close()
    return participants
...
def attack_multiplier(attacker_type, defender_type):
    ...
    # 6. Else, return 1.0
    ######################
    #Dictionary of all attack multipliers
    multipliers = {
        "WaterFire" : 2.5,
        "ElectricWater" : 1.3,
        "GroundElectric" : 2,
    ...
    #if given types have no specified multiplier, return 1.0
    if attacker_type+defender_type in multipliers:
...
def fight(participant1, participant2, first2attack):
    ...
    ######################
    rounds  = 0
    #initialize fighters health
    hp1 = participant1[2]
    hp2 = participant2[2]
    #Fight until death of pokemon
    while(hp1 > 0 and hp2 > 0):
        if first2attack == 1:
            hp2 -= participant1[3]*attack_multiplier(participant1[1], participant2[1])
            
        else:
            hp1 -= participant2[3]*attack_multiplier(participant2[1], participant1[1])
        rounds+=1
        #toggle first2attack
        if first2attack == 1:
            first2attack = 2
    ...
    #declare winner
    if hp1 < hp2:
        return [participant2, rounds]
    else:
        return[participant1, rounds]
...
def tournament(participants):
    ...
    for i in range(len(participants)):
        starting_point+=1
        for j in range(starting_point, len(participants)):
            if fight(participants[i], participants[j], 1)[0] == participants[i]:
                wins[i]+=1
            else:
                wins[j]+=1

            if fight(participants[i], participants[j], 2)[0] == participants[i]:
                wins[i]+=1
            else:
                wins[j]+=1
